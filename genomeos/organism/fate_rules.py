...
What a terminal cell reads:
    ...
    integrated      exposure summed along its lineage path from the zygote. Two ways of computing it are
                    in this file. `exposures` + `integrated` is the **precomputed** one (minutes at the
                    factor's maximum level from `atlas_levels`, called present at >= 20% of the factor's
                    largest path exposure over the embryonic terminal cells); it was what the program read
                    until 2026-09-15, and its threshold is a statistic of the finished run that no
                    deciding cell can have. Since the Body computes `F.exposure(lineage)` itself
                    (BIOLANG-v0.4-ECONOMY.md §7.2a), the program states an absolute threshold in minutes
                    instead and `to_bio_fates` writes that; the precomputed form is kept because
                    `scripts/celegans_fate_reads.py` regenerates it to score the old read through the same
                    Body.
...
# The threshold the rewritten rules state, in the unit the runtime read is in (§7.2a: `exposure` is
# minutes, `mean` a fraction of the window). 15 min is one AB cell cycle at 20 C (Sulston et al. 1983;
# Richards et al. 2013 measure 15 to 16 min for the first AB rounds), so the rule says "the path carried
# this factor across at least one division", not "a reporter was over the line in one frame". The value
# is not fitted and is not load-bearing: every threshold from 1 to 30 min gives the identical program and
# the identical score (1 to 60 min for the cited rules with nothing fitted), measured in
# scripts/celegans_fate_reads.py. Above the plateau the score rises only because the rules claim fewer
# cells, which is buying a number with abstention, so the plateau is where the threshold is chosen.
THRESHOLD_MIN = 15.0
THRESHOLD_BASIS = (
    "threshold 15 min = one AB cell cycle at 20 C (Sulston et al. 1983, Dev Biol 100:64), so the read is "
    "'carried along the lineage path across at least one division'; 1 to 30 min give the identical "
    "program and score, and the cited rules alone are flat to 60 min (scripts/celegans_fate_reads.py)"
)
...
def to_bio_fates(
    ...
    module: str = "organism.celegans.fates",
) -> str:
    ...
    A rule written against the integrated read states the read the Body computes for it,
    `F.exposure(lineage) >= T` or `F.mean(lineage) >= T`, in the unit that read is in (§7.2a: minutes for
    `exposure`, a fraction of the window for `mean`). Nothing is precomputed: the threshold is absolute and
    a deciding cell can hold it. A rule written against the instantaneous read names the factor the reader
    gives the cell, as in v0.3."""
    unit = "minutes carried" if read == "exposure" else "fraction of the window carried"
    lines = [
        "# Generated by scripts/celegans_fate_reads.py (genomeos.organism.fate_rules): terminal fates from",
        f"# the measured factors (Ma et al. 2021), read by the runtime as `F.{read}({window})` — {unit}",
        f"# along the {window} window, threshold {threshold:g}, integrated by the Body itself "
        "(BIOLANG-v0.4-ECONOMY.md §7.2a).",
        "# " + THRESHOLD_BASIS + ".",
        "# Imported after the lineage program, so a factor rule overrides the observed fate of a terminal",
        "# cell and the lookup stays the fallback. Precedence is the `priority` of each decision (first",
        "# rule highest; the lookup has 0) under `regime { fates: first }`, declared by the importer.",
        f"module {module}",
        "import cell_types.bio",
        "",
    ]
    for i, r in enumerate(rules):
        if r.read == "integrated":
            cond = ", ".join(f"{f}.{read}({window}) >= {threshold:g}" for f in r.factors)
            how = f"read as {read} over the {window} window, {THRESHOLD_BASIS}"
        else:
            cond = ", ".join(f"{f} = present" for f in r.factors)
            how = "read as instantaneous presence"
        ...
        lines.append(
            ...
            f'evidence: {kind} "{src}; {how}; factors {SOURCE}"; confidence: {conf} }}'
        )
    return "\n".join(lines) + "\n"
...
def runtime_features(
    ...
    """cell -> factors whose runtime read over `window` reaches `threshold`, from the values the Body
    itself put in each cell's context at its fate decision point. This is the feature set the generated
    rules are learned on, so what a rule is fitted to is exactly what it will read when it runs."""
    suffix = f".{read}({window})"
    return {
        cid: {k[: -len(suffix)] for k, v in r.items() if k.endswith(suffix) and v >= threshold}
...
def to_bio_exposure(
    ...
    """The **precomputed** integrated read, superseded 2026-09-15 by `F.exposure(lineage)` in the runtime
    and kept only so the old read can still be scored through the same Body (the baseline arm of
    scripts/celegans_fate_reads.py). One `express` decision per embryonic terminal cell with the integrated
    calls the rules read. A cell the atlas did not track also gets its inherited instantaneous set, because
    an express decision replaces the measured factors a cell inherited. What makes it precomputed rather
    than mechanism: a factor is called present at a fraction of its largest path exposure *over the
    finished run*, which is a statistic no deciding cell can hold."""
    wanted = set(factors)
    lines = [
