...
def to_bio_fates(rules: list[Rule], integrated_read: bool, module: str = "organism.celegans.fates") -> str:
    """Rules as `differentiate` decisions. The Body fires every applying unfired differentiate decision in
    module order and the last one wins, so the list is written in reverse: the first rule is the last line.
    That a later rule can overwrite an established fate is itself the missing commitment construct."""
    ...
    lines = [
        ...
        "# The Body fires every applying differentiate decision in order and the last wins: precedence runs",
        "# bottom to top.",
        f"module {module}",
