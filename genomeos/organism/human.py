...
def to_bio_tissues(table: dict) -> str:
    ...
    lines = [
        ...
        "# Adult counts, germ-layer splits, growth to the adult count and daily turnover per tissue",
        "# population. Counts, not mechanism.",
        "#",
        "# On the split fractions below: the splits at one decision point run in sequence, so a splitting",
        "# `fraction` is a share of what is left at that point and not of the layer",
        "# (docs/BIOLANG-v0.3.md, populations). The published share of the layer is what Sender & Milo",
        "# 2021 give and what a reader wants to check; the fraction is what the runtime applies. They",
        "# differ after the first split, by a lot -- the last split of a layer reads 1.0000 whether its",
        "# share is 24.5% or 0.00071% -- so every line states both, and neither number is left implied.",
