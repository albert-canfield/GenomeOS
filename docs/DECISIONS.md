...
| D43 | 2026-09-12 | The grammar of the genome is inferred by comparison on two axes, never one: constraint across species (Zoonomia, 241 mammals) says a block matters, constraint within humans (gnomAD Gnocchi, 76,156 genomes) says whether its bases are syntax or value slots, and the four combinations are reported per block as evidence with a confidence, never as a verdict of neutrality. Every library and gene will carry an origin (the deepest clade with an orthologue, Ensembl Compara), every duplication comes from curated data (paralogues, segmental duplications) before any k-mer heuristic, and the product form is one decompiled view per locus that assembles the existing layers with their evidence rather than a new inference | docs/GRAMMAR-BY-COMPARISON.md; ROADMAP.md area J |
