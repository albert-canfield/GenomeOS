...
- **Orthology from Ensembl Compara for the mouse comparison** (area B next step 2): `genome/mouse.py` streams the mouse-side Compara 116 dump once (112 MB; the human-side dump lists 199 species without the reference mouse, a LESSONS row) and keeps the mouse–human orthologue pairs as symbols for the mouse chromosomes fetched (2,176 pairs for chr19 and chr11; `compara_mouse_human_orthology.tsv.gz`); `genomeos mouse` now compares nodes under MGI and under Compara and reports the agreement: on chr19 same human neighbourhood 94.5% (MGI) against 95.5% (Compara), chr11 92.6% against 91.8%; where both sources name orthologues for a gene they name the same human genes 97.3% and 96.3% of the time, Compara covers 65 and 86 genes MGI lacks, MGI 34 and 42 Compara lacks. The node finding does not depend on the orthology source.
- **Measured-RNA panel rows re-run with IMR-90 oriented** (chr21 and chr22, five configurations): the inverted line had been passing antisense candidates over real genes, not merely contributing nothing; oriented, the six-line measured filter's gene precision goes from 62.9% to 72.4% on chr21 and 58.8% to 74.2% on chr22 at the same sensitivity (candidates 143→127 and 289→225), the union from 63.0% to 72.3%, the intersections 97.2%→95.8% (72 candidates, the same 69 genes) and 94.4%→95.1%. The series' ordering stands; SEQUENCE-GRAMMAR.md tables and an "Oriented" paragraph updated.
- **The missense effect predicted rather than ranked** (area D next step 2): `genomeos individual coding --name N --predict` streams DeepMind's AlphaMissense hg38 table once (643 MB, 71.7 million rows, 37 s from the public bucket) and keeps only the person's missense variants under the person's git-ignored directory (`genome/missense.py`; the score on the person's transcript where the table has it, else the highest across transcripts; classes as the model's; confidence capped at 0.7; CC BY-NC-SA, so never in a committed result). HG002: 9,830 missense SNVs, 8,933 scored; 188 likely pathogenic (37 homozygous), 254 ambiguous, 8,659 likely benign, 729 unscored; the inventory, the dossier and the Twin card's coding view now list them strongest first beside UniProt's site annotation, which remains the ranking when no scores exist.
- **Telomere from a BAM read by ranges** (area D next step 1; CRAM needs htslib, so the range route is BAM): `genome/bam_range.py` reads an indexed BAM behind a URL with the standard library (BGZF blocks, the .bai's linear index and pseudo-bins, records), and `genomeos telomere <bam-url> --bai <index>` runs TelSeq's arithmetic without downloading the file: the index gives 6,182,841,975 reads (372,318,760 unmapped), a 100 MB sample of the unmapped tail (910,358 reads, 1,130 with seven TTAGGG repeats) is scaled by that count, and the first and last 10 kb of assembled sequence of each chromosome are read whole (44 of 46 ends carry reads, mean coverage 202.7x; chr5 p and chrX q are telomeric repeat in the assembly itself, 31% and 37% of their reads). HG002: telomere ≈ 2,676 bp per end from 218 MB in 48 requests, inferred at 0.3 (no GC normalisation; a number to compare between people read the same way, not to quote). `telomere_range_HG002.json`; DATA.md note.
- **De novo candidates normalised across the three call sets** (area D next step 3): the trio comparison now normalises every allele before matching (common suffix and prefix trimmed to one anchor base, indels left-aligned along the local reference; `normalise_variant`), since the three GIAB files write one insertion as TGG>TGGG, T>TG and a deletion of two units at different repeat units. HG002 against HG003 and HG004: de novo candidates 12,296 → 1,430 (1,173 SNVs, 257 indels), Mendelian errors 2,930 → 7, inherited 96.4% → 96.9%. What remains is child PASS calls inside both parents' high-confidence regions with no parent row within a base: the true de novos (60 to 100 expected) plus the benchmark files' disagreements; the imported genotypes are unphased, so phasing by parent waits for a phased import. `individual trio` prints the SNV/indel split and the representation used.
- **Genome diff as code review** (improvement pool): `genomeos individual diff --name A --against B|reference` renders two people's protein-changing variants gene by gene as a review (`genome/diff.py`): alleles normalised, `+` only A, `-` only B, the shared count, each line with its consequence on the canonical transcript, zygosity, AlphaMissense class where the person is scored and ClinVar row where screened; genes ordered ClinVar first, then truncations, then predicted class. The coding inventory now keeps every protein-changing variant under the person (`coding_variants.json`); `/api/individual/diff` and a "compare with" control on the Twin card serve it from those files. HG002 against HG003: 9,745 and 9,649 protein-changing variants, 7,513 shared, 2,857 genes differ; only HG002 2,232 (23 truncating, 75 predicted likely pathogenic), only HG003 2,136 (30 truncating; not yet scored). Against the reference, HG002's own edits: 5,344 genes.
- **Polygenic scores from the PGS Catalog** (area D next step 2): `genomeos individual pgs --name N [--score ID ...]` streams a score's harmonised GRCh38 weight table once (`genome/polygenic.py`, cached git-ignored) and sums effect-allele dosages over the person's genotypes: called variants as called, no-call positions inside the trusted regions as homozygous reference against the local FASTA, positions outside them missing and counted, allele mismatches counted; weights `curated`, the sum `derived` at 0.3, a raw score with coverage rather than a percentile (the catalog publishes no population distribution) placed against the other local people, the ancestry caveat carried. Defaults breast cancer (313 variants), LDL (223), height (3,290), coronary artery disease (1.7 million). The trio reads as it should: HG002 sits between HG003 and HG004 on LDL (+0.77 between +0.93 and +0.61), height (54.5 between 55.0 and 53.3) and CAD (−0.82 between −1.14 and −0.48), coverage 90 to 99%. `/api/individual/pgs` and a button on the Twin card read the saved scores; results under the person, never under data/results; DATA.md note.
- **Genome diff at the regulatory level** (area D next step 3): `genomeos individual diff --regulatory --chrom C [--constraint]` (`genome/regdiff.py`) places two people's normalised variants inside the chromosome's ENCODE elements and writes the difference per target gene: the element's class, the CTCF node's inferred target with basis and distance, the AlphaGenome deletion target where the element has been scored, and with `--constraint` the base's own phyloP read by range; constrained and predicted lines first; served by `/api/individual/diff` with a regulatory checkbox on the Twin card. HG002 against HG003 on chr21: 5,730 and 5,579 variants inside the 13,356 elements, 4,352 shared; only HG002 1,378 (14 in a scored element, 27 on a constrained base), only HG003 1,227 (18 scored, 19 constrained). A variant in an element is a candidate, not an effect, and the page says so; the effect per variant is the self-hosted model's job.
- **The dossier carries the polygenic scores**: `individual report` gains a section listing every score computed for the person (trait, publication, variants used, raw score) with the not-a-percentile and ancestry caveats, so the one-page dossier now covers import, checks, ClinVar screen, truncations, coding inventory with predicted missense, trio and polygenic scores.
- **Measured Hi-C boundaries, unblocked**: the 4DN key is in `.env`; the reader now drops the portal's Authorization header on the redirect to S3 (the cause of the 403 with a valid key), and `genomeos domains --chrom chr21 --hic B` ran for five biosources: against GM12878's in situ Hi-C (557 boundaries) 58% of the 227 CTCF-only boundaries sit within 20 kb of a measured one against 42% at random (median 15 kb; 27% of the measured covered), H1-hESC Micro-C 24% against 13%, K562 14% against 8%, HepG2 15% against 8%, IMR-90 12% against 11%. Clearer than the predicted contact map (37% against 28%) and still a proxy; node edges stay at 0.4. `domains_chr21_hic_<biosource>.json` for the five; DATA.md table; ALPHAGENOME.md use 4.
- **Every element in a chr21 node scored per cell line** (area I's first next step, Albert's go on the quota): `scripts/enhancer_targets_all.py --chrom C` (job `enhancer_targets_all_<chrom>`, one entry per chromosome) deletes every enhancer inside a node and keeps each gene's effect on the K562, HepG2, GM12878 and IMR-90 tracks, waiting out quota answers and heartbeating through them; the committed result is the summary, the per-element table stays local (data/knowledge/alphagenome/all_elements). chr21: 12,139 elements, 5,409 requests on top of 6,800 cached, 102 minutes, no quota refusal; 64.6% name a gene (7,846, all with the four cells' values), 5,174 a coding gene; strong 2,182, silencer-like 3,055; the coding target is the nearest TSS in the node for 60.9% and inside the node for 76.5%, the same shape the uniform sample gave. The table is the gene-level closure's input on the whole chromosome (genomeos-c6 reruns it); genomeos-79's chain continues chromosome by chromosome, smallest first.
- **The deletion cache packed per chromosome**: the per-element answers of a finished chromosome fold into one gzipped archive read transparently by `load_cached` (`pack` in `predict/enhancer_target.py`, `scripts/pack_element_cache.py`, only chromosomes whose job says complete, files removed only once every answer is inside). chr21, chr22 and chrY went from 351 MB in 32,804 files to 34.7 MB in three, which is what keeps the genome-wide run inside a 91%-full disk (a million files and 13 GB otherwise). chr22 complete as well (19,708 elements, 66.5% name a gene, nearest TSS 65.3%, inside the node 83.8%) and chrY (907); genomeos-c6's closure replicated on chr22 at 32% against a 25% null, and the wrong-cell genes it flagged turned out to sit in element-dense nodes whose per-cell values separate the lines correctly.
- **The telomere estimate's GC correction** (area D's last Missing item): the range reader now measures the share of reads whose GC matches a telomeric read's (48 to 52%) separately on the unmapped tail and on 200,000 mapped reads from the middle of chromosomes, and reports `telomeric_per_gc_comparable_read` beside the base-pair figure (HG002: 4.3% and 4.1% in the band, index 0.0019 against a plain fraction of 0.00008, 56 range requests). The base-pair figure keeps the derivation that holds; TelSeq's own scale multiplies the GC ratio by a constant fitted to Southern blots, which is not derivable here, so it is not claimed as base pairs, and both figures are stated as comparable between people read the same way. An honest number beats a familiar-looking one: the GC ratio scaled the naive way gave 64 kb, which no human telomere is.
- **The all-elements scoring was latency-bound, not quota-bound**: no request in the chr19 run was ever refused, yet it managed 51 requests a minute, because it waited for each round trip in turn. The scorer now keeps eight requests in flight (`--workers`, a scorer per worker with the client's own five-minute timeout in place of the signal watchdog, the sequence reader behind a lock) and a quota answer to any worker holds all of them, escalating to an hour if the refusals persist. Measured on chr20: 96 fresh requests in 11 s at twelve workers, 520 a minute, no waits; chr19's remaining 26,000 elements go from nine hours to under one. The per-chromosome entries also stop auto-healing: each spends the shared quota and their order matters, so restarts belong to the chain, not to a supervisor tick that would start a chromosome nobody asked for on the next `serve` restart (which happened once today, on chr1).
- **Hangs survived rather than prevented**: twice on 2026-09-13 a scoring run parked with every worker quiet, the process alive, 0% CPU and nothing in the log, once for 2 h 19 min; no quota answer, no timeout, no traceback, and the per-request watchdog did not catch it. The run now keeps a bounded window of tasks (four per worker rather than 29,000 queued), logs a waiting line every minute of silence, and if nothing is scored for ten minutes it saves and exits non-zero so whoever sequences the chromosomes restarts it, resuming from the cache. The cause is still unknown, so the guard is on the symptom: a run that cannot make progress ends instead of sitting alive.
...
## 2026-09-12 — one locus across species

Area J step 7, the experiment the DNA-as-code conversation ended on.
`genomeos across` aligns a human element to mouse, opossum, chicken, frog and
zebrafish through Compara's pairwise alignments and asks which transcription
factors hold the same aligned site in every species. The first version
counted a motif anywhere in the aligned sequence and called 170 factors
conserved across the SHH limb enhancer, the ZRS, which says only that the
sequence is 88% identical; mapping each hit through the alignment columns
cut that to 23 of 606, and reviewing the code caught two more defects the
tests had not (gaps stripped before mapping, a factor list capped at 60).
What holds its site from human to fish is the HOX, PBX and MEIS class with
an ETS site, the known logic of the ZRS, with HAND2 held in the three
amniotes. The eye-colour enhancer of OCA2 aligns only to mammals, is
constrained among people far more than across mammals, and keeps the MiT
E-box class and a SOX10 site. The abstraction survives its first test; the
