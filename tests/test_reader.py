...
def test_poised_promoters_are_not_read(tmp_path, monkeypatch):
    import gzip

    from genomeos.genome import epigenome, reader

    monkeypatch.setattr(reader, "RESULTS", tmp_path)
    monkeypatch.setattr(epigenome, "CACHE", tmp_path / "epi")
    with gzip.open(tmp_path / f"dnase_{slug('H1')}_c.bed.gz", "wt") as fh:
        fh.write("# test\n900\t1200\t5.0\n19900\t20300\t4.0\n")

    def peaks(mark, rows):
        p = epigenome.peaks_path("H1", mark, "c")
        p.parent.mkdir(parents=True, exist_ok=True)
        with gzip.open(p, "wt") as fh:
            fh.write("# test\n" + "".join(f"{s}\t{e}\t1.0\n" for s, e in rows))

    peaks("H3K27me3", [(800, 1300)])  # on A: open and Polycomb-marked
    peaks("H3K27ac", [(19800, 20400), (39800, 40400)])  # on B (open) and C (closed)
    peaks("H3K4me3", [(800, 1300), (39800, 40400)])
    ann = _Ann([_gene("g1", "A", 1000, 5000), _gene("g2", "B", 20000, 25000), _gene("g3", "C", 40000, 45000)])
    r = read_chromosome("H1", "c", ann, [Domain("c:D1", "c", 0, 60000)], [])
    assert r["marks_used"] and r["poised_genes"] == ["A"] and r["genes_poised"] == 1
    assert r["genes_read"] == 2 and r["genes_read_open"] == 2 and r["_read_all"] == ["B", "C"]
    assert r["read_by_marks"] == ["C"] and r["genes_read_by_marks"] == 1
    assert r["silent_genes"] == ["A"] and r["genes_closed"] == 0 and r["genes_silent"] == 1
    assert "not poised" in r["evidence"]["read"]
    plain = read_chromosome("H1", "c", ann, [Domain("c:D1", "c", 0, 60000)], [], marks=False)
    assert plain["genes_read"] == 2 and plain["genes_poised"] is None and not plain["marks_used"]
