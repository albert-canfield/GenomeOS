...
"""Grade a third set of published enhancer-gene loci against the machinery.

    uv run python scripts/loci_third.py --plan              # reach and cost, free, no request
    uv run python scripts/loci_third.py --intervals --run --quota-handed
    uv run python scripts/loci_third.py --score             # read and score, no request

The roadmap item is "the known-locus benchmark with more loci, not more readings of the one it has",
and section 19's own "left undone" says which loci: a set that tests the direction axis in BOTH
directions and whose element lengths span the panel's range. The loci, the hit rules, the
denominators, the controls and the predictions are registered in `genomeos/benchmark/loci_third.py`
before anything is scored; this file only drives them.

Three steps, the free ones first:

  --plan       `loci.read_reach` per locus (gene BODY against the scorer's 1 Mb input, never TSS
               distance), then whether the finished all-chromosome sweep has already deleted an
               element inside the published interval - in which case the element-level answer costs
               nothing. Prints the request count before a request exists.
  --intervals  deletes the published elements no annotation covers, one request each, through the
               same `Context.score_region` the other two sets' stated intervals use. Skips anything
               the reach filter calls unaskable: a request there buys a certain negative.
...
def show_plan(rows: list[dict[str, Any]]) -> None:
    for r in rows:
        if not r["askable"]:
            ...
            continue
        say(
            f"{r['locus']}: graded, {r['length']} bp, {r['direction']},"
            f" targets in reach {r['targets_in_reach']},"
            f" {r['ccres_over_element']} cCRE(s) over the element,"
            f" {r['already_scored_over_element']} already deleted, {r['requests']} request(s)"
        )
    say(
        f"{sum(1 for r in rows if r['graded'])} graded of {len(rows)} loci;"
...
def score_intervals(rows: list[dict[str, Any]]) -> dict[str, Any]:
    """Delete the published elements the sweep has not already covered, one request each."""
    t0 = time.time()
    ...
    by_name = third.by_name()
    with the_key(f"known-locus third set, {len(asked)} stated intervals"):
        for row in asked:
            ...
            read.append(
                {
                    ...
                    "published_direction": by_name[row["locus"]].direction,
    ...
    return {
        "result": third.INTERVALS,
        "elements": elements,
        "read": read,
        "note": (
            "AlphaGenome deletions of intervals genomeos/benchmark/loci_third.py STATED from a"
            " publication, which the finished sweep had not already covered. Read by"
            " loci._deletion_rows and labelled `stated_interval`, so these rows are never pooled with"
            " registry ones. loci.STATED_INTERVAL_RESULTS must list this result's name for that to"
            " happen: see loci_third.NEEDED_LOCI_HUNK and loci_third.register_stated_intervals"
        ),
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument("--plan", action="store_true", help="reach and cost per locus; no request")
    ...
    if args.intervals:
        ...
        say(f"saved {save_result(third.INTERVALS, out)}")

    if args.score:
        out = third.run(network=not args.no_network, progress=say)
        a = out["aggregate"]
        say(
        ...
        print(json.dumps(out["directions"], indent=1))
        print(json.dumps(out["positive_control"], indent=1))
        print(json.dumps(out["beside_the_other_two"], indent=1))
    return 0
